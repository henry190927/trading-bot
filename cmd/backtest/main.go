...
func main() {
	...
	maxHold := flag.Int("hold", 24, "max bars to hold before timing out. A/B'd 2026-09-22 over 8/12/24/48/72 x 60/90/120d x BTC/ETH/XAU/XAG/SOL: 18 of 20 gate runs FAIL against the 24 default, which therefore stays. The two passes are SOL 1h at 48 and 72 — adjacent params, and netR rises monotonically there from 24 upward in all three windows, which is mechanism-consistent because SOL 1h runs StructMomentum and a momentum strategy wants room to run. NOT shipped, and the reason changed once it was chased. Out-of-sample at 180/240/300d it holds up (h48 PASSes the gate, medR/t +0.397, and h24/h48/h72 keep the SAME trade counts — the gain is purely trades that h24 was timing out). But there is nowhere to ship it: -hold is a BACKTEST timeout and autotrade.Rule has no live equivalent, so adding one would be the decorative knob its own comment warns against. The effect is also SOL-1h-only — SOL 2h, SUI 1h and LINK 2h are inert because no trade there lasts 24 bars (verified: h24 == h200 while h2/h4/h8 differ, so the flag works). A follow-on hypothesis that h24 biases SM-vs-MR A/Bs against momentum was tested directly and is FALSE: the SOL 1h gap goes 15.76->3.62 (90d), 25.64->22.72 (180d), 32.91->36.82 (240d) moving h24->h72, so longer holds help both arms and the earlier NEAR/SUI controls do not generalise.")
